(function (document, window) {
  ...
  function createPager(element, parameters) {
    ...
    for (var key in previousLinks) {
      ...
      link.innerHTML =
        '<span class="js-page-number visually-hidden">' +
        previousLinks[key] +
        "</span>" +
        key;
    ...
    if (pagesDifference < pagesToShow) {
      startIndex =
        parameters.currentPage - (pagesToShow - pagesDifference - 1) || 1;
    }
    var link;
    for (
      var i = startIndex;
      i < parameters.currentPage + pagesToShow && i <= parameters.pagesNumber;
      i++
    ) {
      link = document.createElement("a");
      link.href = url + i;
    ...
    var nextLinks = {
      "&gt;":
        parameters.currentPage === parameters.pagesNumber
  ...
  function init() {
    ...
    document
      .getElementsByClassName("js-thumbnails__pager")[0]
